...
# Removes a scope exclusion -- a computer group by ID, a user by username, or
# both -- from every Jamf Pro policy listed in a CSV file. Partner script to
# Add_Policy_Scope_Exclusion.sh; same arguments, same safety model.
#
...
Usage:
  Remove_Policy_Scope_Exclusion.sh --csv <file> [--group-id <n>] [--username <name>] [options]
...
At least one of:
  ...
  --username <name>        User to un-exclude, by username. Allowed characters:
...
confirm_token="REMOVE-${policies_total}-${group_id:-none}-${username:-none}"
