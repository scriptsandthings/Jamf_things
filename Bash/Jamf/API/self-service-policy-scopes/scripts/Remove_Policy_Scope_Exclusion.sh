...
# Removes a scope exclusion -- a computer group by ID, a Jamf Pro user group by
# name or ID, a user by username, or any combination -- from every Jamf Pro
# policy listed in a CSV file. Partner script to Add_Policy_Scope_Exclusion.sh;
# same arguments, same safety model.
#
# --user-group addresses scope/exclusions/jss_user_groups, the Jamf Pro user
# group. The limitation scripts spell the flag the same way and mean the
# directory service group in limitations/user_groups. Different Jamf objects;
# see CLAUDE.md, "The scope model, which is not symmetric".
#
...
usage() {
	cat <<'EOF'
Remove a scope exclusion from the Jamf Pro policies listed in a CSV.

Usage:
  Remove_Policy_Scope_Exclusion.sh --csv <file> [--group-id <n>] [--username <name>]
...
At least one of:
  --group-id <n>           Computer group to un-exclude, by Jamf Pro ID. Works
                           for both smart and static groups -- they share an ID
                           space and the same scope element.
  --username <name>        Directory or local user to un-exclude, by username.
                           Allowed: letters, digits, spaces and . _ - @
  --user-group <name>      Jamf Pro user group to un-exclude, by name. Resolved
                           to its ID before anything is written; a name the
                           server does not know is refused.
...
Exit codes:
  0  every policy in the CSV was already clear of the exclusion or was updated
...
ProcessPolicy() {
	...
	if [[ "$kind" = "group" ]]; then
		...
		# 11.32.0 on 2026-09-12: the stored entry carries <id> and a <name> the
		# server filled in, so the match is on id. The same group can be a
		# deployment target in scope/jss_user_groups at the same time;
		# RemoveScopeEntry is bounded to the section, so removing the exclusion
	...
	if [[ "$existing" -eq 0 ]]; then
		log_line "ALREADY  ${policy_id} (${policy_name}): exclusion is not on this policy, nothing to do."
		policies_already=$((policies_already + 1))
		return
	fi

	# Normalise before line-oriented editing. The API's own formatting is not
	# guaranteed, and the awk removal is line-based.
	...
	if ! new_scope=$(RemoveScopeEntry "$scope_xml" exclusions "$container" "$entry" "$match_element" "$value") ||
	   [[ -z "$new_scope" ]]; then
		log_line "FAILED   ${policy_id} (${policy_name}): could not remove the exclusion from the scope."
		policies_failed=$((policies_failed + 1))
		return
	fi
	...
	if [[ "$apply_changes" != "yes" ]]; then
		log_line "WOULD REMOVE ${policy_id} (${policy_name}): ${match_element} ${value} from exclusions/${container}"
	...
	# intended, and this is the check that proves the entry is gone rather
	...
	if [[ "$verify_count" -eq 0 ]]; then
		log_line "UPDATED  ${policy_id} (${policy_name}): removed ${match_element} ${value} from exclusions/${container}"
		policies_updated=$((policies_updated + 1))
	else
		log_line "FAILED   ${policy_id} (${policy_name}): PUT returned ${http_code} but the exclusion is still present on read-back."
...
confirm_token="REMOVE-${policies_total}-${group_id:-none}-${username:-none}-${user_group_name:-${user_group_id:-none}}"
...
log_line "Remove_Policy_Scope_Exclusion.sh ${SCRIPT_VERSION}"
...
log_line "Not excluded:     ${policies_already}"
