...
# Disables every Jamf Pro policy listed in a CSV file.
#
# This sets <general><enabled> to false. The policy keeps everything it has --
# scope, category, triggers, Self Service entry -- and stops doing any of it. It
# disappears from Self Service on the targeted Macs and stops firing on its
# triggers. Re-enable with Enable_Policy.sh; nothing here is destructive.
#
# Unlike enabling, disabling cannot start anything running, so there is no
# equivalent of Enable_Policy.sh's --allow-auto-trigger guard here. Each
# dry-run line still names the triggers, because what a policy runs on is what
# you are switching off.
#
...
# What differs from Enable_Policy.sh, as data. It is not the only difference:
# Enable also carries the --allow-auto-trigger guard (the usage text, the
# argument that accepts it, and the REFUSED block in ProcessPolicy), and this
# script refuses that flag below. Diff the two files before editing either.
target_state="false"
action_present="DISABLE"
action_past="DISABLED"
action_past_lower="disabled"
token_prefix="DISABLE"
backup_prefix="disable"
...
usage() {
	cat <<'EOF'
Disable the Jamf Pro policies listed in a CSV.

Usage:
  Disable_Policy.sh --csv <file> [options]
...
Options:
  ...
  --backup-dir <dir>       Where to write per-policy backups.
                           Default: ./disable-backups-YYYYmmdd-HHMMSS
...
Exit codes:
  0  every policy in the CSV was already disabled or was disabled
...
while [[ $# -gt 0 ]]; do
	case "$1" in
		...
		--allow-auto-trigger)
			# Refused rather than ignored. The flag exists on Enable_Policy.sh,
			# where it permits switching on a policy that then runs by itself.
			# Accepting it here would let someone believe a guard applied.
			echo "ERROR! --allow-auto-trigger belongs to Enable_Policy.sh."
			echo "       Disabling a policy cannot start anything running, so"
...
log_line "Disable_Policy.sh ${SCRIPT_VERSION}"
