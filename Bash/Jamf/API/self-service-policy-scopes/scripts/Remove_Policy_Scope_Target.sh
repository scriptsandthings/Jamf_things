...
# Removes a scope target -- a computer group by ID, a computer by ID, or both --
# from every Jamf Pro policy listed in a CSV file. Partner to
# Add_Policy_Scope_Target.sh; same arguments plus --allow-empty-scope, same
# safety model.
#
...
usage() {
	cat <<'EOF'
Remove a scope target from the Jamf Pro policies listed in a CSV.

Usage:
  Remove_Policy_Scope_Target.sh --csv <file> [--group-id <n>] [--computer-id <n>] [options]
...
At least one of:
  --group-id <n>           Computer group to un-target, by Jamf Pro ID. Works
                           for both smart and static groups -- they share an ID
                           space and the same scope element.
  --computer-id <n>        Individual computer to un-target, by Jamf Pro ID.

Note: users are not targets. Jamf Pro scopes a policy to machines and then
narrows by user with a limitation -- see Remove_Policy_Scope_Limitation.sh.
...
Options:
  ...
  --allow-empty-scope      Permit a removal that leaves a policy with no targets
                           at all. Refused by default: such a policy never runs
                           again, and it is the one change here that looks like
                           nothing happened rather than like a mistake.
...
Exit codes:
  0  every policy in the CSV was already clear of the target or was updated
...
while [[ $# -gt 0 ]]; do
	case "$1" in
		...
		--allow-empty-scope) allow_empty_scope="yes"; shift ;;
		...
		--username|--user-group|--user-group-id)
			...
			echo "       Did you mean Remove_Policy_Scope_Limitation.sh?"
			exit 3
			;;
...
ProcessPolicy() {
	...
	# Captured for the empty-scope guard below: with All Computers set, a policy
	# with no listed targets still runs everywhere.
	...
	if [[ "$existing" -eq 0 ]]; then
		log_line "ALREADY  ${policy_id} (${policy_name}): target is not on this policy, nothing to do."
	...
	if ! new_scope=$(RemoveScopeEntry "$scope_xml" targets "$container" "$entry" "id" "$value") ||
	   [[ -z "$new_scope" ]]; then
		log_line "FAILED   ${policy_id} (${policy_name}): could not remove the target from the scope."
		policies_failed=$((policies_failed + 1))
		return
	fi

	# A policy with no targets never runs again. That is a silent outcome -- the
	# policy still exists, still looks configured, and simply stops reaching any
	# Mac -- so it is refused unless asked for explicitly. The fragment is
	# wrapped in <policy> so the library's absolute XPath matches it.
	remaining_targets=$(CountScopeTargets "<policy>${new_scope}</policy>")
	if [[ "$all_computers" != "true" ]] && [[ "$remaining_targets" -eq 0 ]] && [[ "$allow_empty_scope" != "yes" ]]; then
		log_line "REFUSED  ${policy_id} (${policy_name}): removing this target would leave the policy with no targets at all. --allow-empty-scope overrides."
		policies_skipped=$((policies_skipped + 1))
		return
	fi
	...
	if [[ "$apply_changes" != "yes" ]]; then
		log_line "WOULD REMOVE ${policy_id} (${policy_name}): id ${value} from targets/${container}"
	...
	if [[ "$verify_count" -eq 0 ]]; then
		log_line "UPDATED  ${policy_id} (${policy_name}): removed id ${value} from targets/${container}"
		policies_updated=$((policies_updated + 1))
	else
		log_line "FAILED   ${policy_id} (${policy_name}): PUT returned ${http_code} but the target is still present on read-back."
...
confirm_token="REMOVE-TARGET-${policies_total}-${group_id:-none}-${computer_id:-none}"
...
log_line "Remove_Policy_Scope_Target.sh ${SCRIPT_VERSION}"
...
log_line "Not targeted:     ${policies_already}"
