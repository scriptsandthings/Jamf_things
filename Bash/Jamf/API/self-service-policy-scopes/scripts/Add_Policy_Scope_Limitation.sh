...
# Adds a scope limitation -- a user by username, or a user group by name or by
# ID -- to every Jamf Pro policy listed in a CSV file. Partner to
# Remove_Policy_Scope_Limitation.sh; same arguments, same safety model.
# Sibling of Add_Policy_Scope_Exclusion.sh with different selectors, because
# limitations take users and user groups and never computer groups.
#
...
usage() {
	cat <<'EOF'
Add a scope limitation to the Jamf Pro policies listed in a CSV.

Usage:
  Add_Policy_Scope_Limitation.sh --csv <file> [--username <name>]
...
At least one of:
  --username <name>        User to limit to, by username. Allowed characters:
                           letters, digits, and . _ - @
  --user-group <name>      User group to limit to, by name. Allowed characters:
                           letters, digits, spaces, and . _ - @
                           This is the option for an Entra group arriving
                           through a Cloud Identity Provider.
  --user-group-id <n>      User group to limit to, by Jamf Pro ID. Use this only
...
use Add_Policy_Scope_Exclusion.sh instead.
...
Exit codes:
  0  every policy in the CSV already had the limitation or was updated
...
while [[ $# -gt 0 ]]; do
	case "$1" in
		...
		--group-id)
			...
			echo "       and iBeacon. Did you mean --user-group, or Add_Policy_Scope_Exclusion.sh?"
			exit 3
			;;
...
ProcessPolicy() {
	...
	case "$kind" in
		...
		group-name)
			...
			entry="user_group"
			match_element="name"
			new_node="<user_group><name>${value}</name></user_group>"
			;;
		group-id)
			container="user_groups"
			entry="user_group"
	...
	if [[ "$existing" -gt 0 ]]; then
		log_line "ALREADY  ${policy_id} (${policy_name}): limitation already present, nothing to do."
	...
	if ! new_scope=$(InsertScopeEntry "$scope_xml" limitations "$container" "$new_node") ||
	   [[ -z "$new_scope" ]]; then
		log_line "FAILED   ${policy_id} (${policy_name}): could not place the limitation in the scope."
		policies_failed=$((policies_failed + 1))
		return
	fi
	...
	if [[ "$apply_changes" != "yes" ]]; then
		log_line "WOULD ADD ${policy_id} (${policy_name}): ${new_node} to limitations/${container}"
	...
	# and this is the check that catches a wrong element shape rather than
	...
	if [[ "$verify_count" -gt 0 ]]; then
		log_line "UPDATED  ${policy_id} (${policy_name}): added ${match_element} ${value} to limitations/${container}"
		policies_updated=$((policies_updated + 1))
	else
		log_line "FAILED   ${policy_id} (${policy_name}): PUT returned ${http_code} but the limitation is not present on read-back."
		if [[ "$container" = "user_groups" ]]; then
			# Jamf product issue PI-005747: the Classic API can return an empty
			# <user_groups/> under <limitations> for a policy whose UI plainly
			# shows a user-group limitation. If that is what happened here the
			# write landed and only the proof failed, so look before restoring.
			log_line "         Known Jamf issue PI-005747 can hide user-group limitations from the API. Check the policy's Scope > Limitations in Jamf Pro before restoring."
		fi
...
confirm_token="ADD-LIMIT-${policies_total}-${user_group_name:-${user_group_id:-none}}-${username:-none}"
...
log_line "Add_Policy_Scope_Limitation.sh ${SCRIPT_VERSION}"
...
log_line "Already limited:  ${policies_already}"
