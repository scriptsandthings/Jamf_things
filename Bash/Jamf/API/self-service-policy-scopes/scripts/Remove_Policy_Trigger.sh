...
# Removes a trigger from every Jamf Pro policy listed in a CSV file.
#
# A policy's triggers are seven separate fields under <general>: six booleans
...
# that "jamf policy -event <name>" matches. This script turns one boolean off,
# or clears the custom event.
#
# Removing a trigger cannot start anything running, so there is no equivalent of
# Add_Policy_Trigger.sh's --allow-auto-trigger guard here. It can stop something:
# a policy whose last automatic trigger is removed, and which is not a Self
# Service policy, becomes unreachable -- it stays enabled and never runs again.
# That case is reported but not refused, because the policy is still visible and
# the change is one command to undo.
#
# --custom-event names the event to remove, and the policy is skipped unless its
# custom event actually matches. Clearing trigger_other blindly would wipe an
# unrelated event off any policy in the CSV that happened to have one.
#
...
usage() {
	cat <<'EOF'
Remove a trigger from the Jamf Pro policies listed in a CSV.

Usage:
  Remove_Policy_Trigger.sh --csv <file> (--trigger <name> | --custom-event <event>) [options]
...
Exactly one of:
  --trigger <name>         One of: checkin, startup, login, logout,
                           network-state-change, enrollment-complete.
                           "recurring-check-in" is accepted for checkin.
                           "checkin" is Recurring Check-in. "logout" is a legacy
                           field Jamf Pro's UI no longer exposes; it is accepted
                           because the API still carries it.
  --custom-event <event>   The custom event to clear. A policy whose custom
                           event is something else is skipped, not cleared.
...
Options:
  ...
  --include-non-self-service
                           ...
                           it should not silently disarm a background policy.
  --help                   This text.

Exit codes:
  0  every policy in the CSV already lacked the trigger or was updated
...
while [[ $# -gt 0 ]]; do
	case "$1" in
		...
		--allow-auto-trigger)
			# Refused rather than ignored. The flag exists on
			# Add_Policy_Trigger.sh, where it permits arming a policy that then
			# runs by itself. Accepting it here would let someone believe a
			# guard applied.
			echo "ERROR! --allow-auto-trigger belongs to Add_Policy_Trigger.sh."
			echo "       Removing a trigger cannot start anything running, so"
			echo "       there is nothing here for it to override."
			exit 3
			;;
...
ProcessPolicy() {
	...
	# Only the named event is cleared. Clearing trigger_other blindly would wipe
	# an unrelated event off any policy in the CSV that happened to have one,
	# and nothing downstream would show that it had gone.
	if [[ "$trigger_element" = "trigger_other" ]] && [[ "$current_value" != "$custom_event" ]]; then
		log_line "SKIPPED  ${policy_id} (${policy_name}): custom event is '${current_value:-none}', not '${custom_event}'."
		policies_skipped=$((policies_skipped + 1))
		return
	fi
	...
	# A policy with no automatic trigger left and no Self Service entry stays
	# enabled and never runs again. Reported, not refused: unlike an empty scope
	# the policy is still visible in the UI, and this is one command to undo.
	# Wrapped in <policy> so the library sees a well-formed document with a
	# <general> section. The fragment carries no <self_service>, which is why
	# self_service_flag is tested separately on the next line.
	remaining_auto=$(GetPolicyTriggers "<policy>${new_general}</policy>")
	if [[ "$remaining_auto" = "none" ]] && [[ "$self_service_flag" != "true" ]]; then
		log_line "         note: ${policy_id} (${policy_name}) will have no trigger and no Self Service entry -- it stays enabled and nothing can run it."
	fi

	if [[ "$apply_changes" != "yes" ]]; then
		log_line "WOULD ${action_present} ${policy_id} (${policy_name}): ${trigger_label}; enabled=${policy_enabled:-none}, was running on: ${triggers_before}"
...
log_line "Remove_Policy_Trigger.sh ${SCRIPT_VERSION}"
